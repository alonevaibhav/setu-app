...
import '../Controller/court_commission_preview_controller.dart';
import '../Controller/main_controller.dart';

class CourtCommissionPreviewStep extends StatelessWidget {
  final int currentSubStep;
  final CourtCommissionCaseController mainController;

  const CourtCommissionPreviewStep({
    Key? key,
    required this.currentSubStep,
    required this.mainController,
  }) : super(key: key);

  CourtCommissionPreviewController get controller => Get.put(CourtCommissionPreviewController(), tag: 'court_commission_preview');
  ...
  Widget _buildCourtCommissionSection() {
    return buildSection(
      title: 'Court Commission Information',
      ...
      children: [
        ...
        if (controller.isNotEmpty(controller.commissionOrderNumber))
          buildInfoRow('Commission Order Number', controller.commissionOrderNumber),
        if (controller.isNotEmpty(controller.commissionDate))
          buildInfoRow('Commission Date', controller.commissionDate),
        if (controller.isNotEmpty(controller.civilClaim))
          buildInfoRow('Civil Claim', controller.civilClaim),
        if (controller.isNotEmpty(controller.issuingOffice))
          buildInfoRow('Issuing Office', controller.issuingOffice),
        if (controller.commissionOrderFiles.isNotEmpty)
          buildFileRow('Commission Order Documents', controller.commissionOrderFiles, controller),
      ],
    );
  }
  ...
  Widget _buildSubmitButton() {
    return Obx(() => Container(
      ...
      child: ElevatedButton(
        onPressed: controller.isSubmitting.value ? null : controller.submitCourtCommissionSurvey,
        ...
        child: controller.isSubmitting.value
          ...
          children: [
            ...
            : Text(
          'Submit Court Commission Survey',
          style: TextStyle(
            fontSize: 16.sp,
            fontWeight: FontWeight.w600,
            color: Colors.white,
