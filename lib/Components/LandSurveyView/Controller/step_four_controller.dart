...
//   final totalPlotNumberController = TextEditingController(text: '1'); // Default value 1 for Non-agricultural
//   final knotCountController = TextEditingController(text: '1'); // NEW: Default value 1 for Counting by number of knots
//
//   // Dropdown Values
...
//   };
//
//   // Non-agricultural fee structure (same structure as Stomach)
...
//   };
//
//   // NEW: Counting by number of knots fee structure
...
//   }
//
//   // Initialize calculation type properly
...
//     knotCountController.dispose(); // NEW: Dispose knot count controller
//     super.onClose();
//   }
//
...
//     });
//
//     // NEW: Listen for knot count changes (for Counting by number of knots calculations)
...
//   }
//
//   // Non-agricultural fee calculation
//   void _calculateNonAgriculturalFee(String duration, String holderType, String? locationCategory) {
//     String key;
//
//     // For companies/institutions, location category is not needed in the key
//     if (holderType.contains('कंपन्या')) {
...
//     print('Available keys: ${nonAgriculturalFeeStructure.keys}');
//
...
//
//       // Calculate total fee: base fee + (plot numbers - 1) * increment
//       final totalFee = baseFee + ((plotNumbers - 1) * increment);
//       calculationFeeController.text = '₹${totalFee}';
//
//       print('Non-agricultural calculation: Base=$baseFee, PlotNumbers=$plotNumbers, Increment=$increment, Total=$totalFee');
//     } else {
...
//   }
//
//   // NEW: Counting by number of knots fee calculation
//   void _calculateKnotCountingFee(String duration, String holderType, String? locationCategory) {
//     String key;
//
//     // For companies/institutions, location category is not needed in the key
//     if (holderType.contains('कंपन्या')) {
...
//     print('Available keys: ${knotCountingFeeStructure.keys}');
//
...
//
//       // Calculate total fee: base fee + (knot count - 1) * increment
//       final totalFee = baseFee + ((knotCount - 1) * increment);
//       calculationFeeController.text = '₹${totalFee}';
//
//       print('Knot counting calculation: Base=$baseFee, KnotCount=$knotCount, Increment=$increment, Total=$totalFee');
//     } else {
...
//             selectedHolderType.value == 'धारक(शेतकरी)'); // NEW: Added knot counting condition
//   }
//
...
//     print('StepFour - requiresTotalPlotNumber check: ${selectedCalculationType.value}');
//     bool requires = selectedCalculationType.value == 'Non-agricultural';
//     print('StepFour - requiresTotalPlotNumber result: $requires');
//     return requires;
//   }
//
//   // NEW: Check if current calculation type requires knot count
//   bool requiresKnotCount() {
//     print('StepFour - requiresKnotCount check: ${selectedCalculationType.value}');
//     bool requires = selectedCalculationType.value == 'Counting by number of knots';
...
//     }
//
//     // For Hddkayam, Stomach with farmer holder type, Non-agricultural with farmer holder type, or Knot counting with farmer holder type, location category is required
...
//     }
//
//     // For Non-agricultural, total plot number is required and should be > 0
...
//     }
//
//     // NEW: For Knot counting, knot count is required and should be > 0
...
//           }
//         }
//         if (requiresKnotCount()) { // NEW: Validation for knot count
//           final knotCount = int.tryParse(knotCountController.text);
//           if (knotCount == null || knotCount <= 0) {
...
//     }
//
//     // Add total plot number only for Non-agricultural calculation
//     if (requiresTotalPlotNumber()) {
//       data['total_plot_number'] = int.tryParse(totalPlotNumberController.text);
//     }
//
//     // NEW: Add knot count only for Knot counting calculation
//     if (requiresKnotCount()) {
...
//     knotCountController.text = '1'; // NEW: Reset knot count controller
//   }
// }
...
class StepFourController extends GetxController with StepValidationMixin, StepDataMixin {
  ...
  final abdominalSectionController = TextEditingController(text: '1'); // Default value 1
  final totalPlotNumberController = TextEditingController(text: '1'); // Default value 1 for Non-agricultural
  final knotCountController = TextEditingController(text: '1'); // Default value 1 for Counting by number of knots
  ...
  final Map<String, int> integrationCalculationFeeMap = {
    ...
    'Integration calculation_Fast Track_धारक(शेतकरी)_म.न.पा./न.पा. अंतर्गत': 12000,
    'Integration calculation_Fast Track_धारक(शेतकरी)_म.न.पा./न.पा. बाहेरील': 8000,
    'Integration calculation_Regular_कंपन्या/इतर संस्था/ विविध प्राधिकरणे/ महामंडळ व भूसंपादन संयुक्त मोजणी': 3000,
    'Integration calculation_Fast Track_कंपन्या/इतर संस्था/ विविध प्राधिकरणे/ महामंडळ व भूसंपादन संयुक्त मोजणी': 12000,
  };
  ...
  void _calculateFee() {
    ...
    if (calculationType != null && duration != null && holderType != null) {
      if (calculationType == 'Hddkayam' && locationCategory != null) {
        ...
        _calculateIntegrationCalculationFee(calculationType, duration, holderType, locationCategory);
      } else {
        calculationFeeController.text = '';
      }
    } else {
  ...
  void _calculateIntegrationCalculationFee(String calculationType, String duration, String holderType, String? locationCategory) {
    String key;
