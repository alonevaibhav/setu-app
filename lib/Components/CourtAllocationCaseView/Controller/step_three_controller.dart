...
//       'area': '',
//       'selectedVillage':
//           'Select Village', // Individual village selection per entry
//     };
...
//     if (surveyEntries.length > 1 &&
//         index >= 0 &&
//         index < surveyEntries.length) {
...
//             'surveyNoController':
//                 TextEditingController(text: entryData['surveyNo'] ?? ''),
//             'shareController':
//                 TextEditingController(text: entryData['share'] ?? ''),
//             'areaController':
...
class CalculationController extends GetxController
      ...
      if (entriesData != null && entriesData.isNotEmpty) {
        ...
        for (var entryData in entriesData) {
          final entry = {
            ...
            TextEditingController(text: entryData['surveyNo'] ?? ''),
            'villageController':
            TextEditingController(text: entryData['village'] ?? ''),
            'areaController':
            TextEditingController(text: entryData['area'] ?? ''),
            'surveyNo': entryData['surveyNo'] ?? '',
  ...
  bool hasVillageInEntries(String village) {
    ...
    entry['village']?.toString().toLowerCase() == village.toLowerCase());
  }

  // Get all unique villages entered across entries
  List<String> getEnteredVillages() {
    final villages = <String>[];
    for (var entry in surveyEntries) {
      final village = entry['village']?.toString().trim() ?? '';
  ...
  double getTotalAreaForVillage(String village) {
    double total = 0.0;
    for (var entry in surveyEntries) {
      if (entry['village']?.toString().toLowerCase() == village.toLowerCase()) {
        final areaStr = entry['area']?.toString() ?? '';
        final area = double.tryParse(areaStr) ?? 0.0;
        total += area;
      }
  ...
  // Get village name for specific entry
  String getVillageName(int index) {
    if (index >= 0 && index < surveyEntries.length) {
      return surveyEntries[index]['village']?.toString() ?? '';
    }
    return '';
  }
